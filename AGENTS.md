...
| `inventory` | Goods receipt + stock ledger + goods issue + FIFO reservation helpers + branch stock + send-to-warehouse (D43): `GoodsReceipt`/`GoodsIssue`/`BranchReceipt`/`BranchWarehouseShipment`/`StockMovement`/`BranchStockMovement`/`BranchItemStock`, services, console API, admin, tests |
| `branches` | Tenancy + catalog (Slices 1–2) + company-wide commercial mode (D37): `Branch`/`BranchMembership`/`BranchCommercialSettings`, `ActiveBranchMiddleware`, `/branch/select/` picker, `/branch/catalog/` + API, admin, services, tests |
...
| `inbox` | Home new-document badges (D45): `SectionCursor` watermarks for work queues; dashboards consume counts |
...
- **Branches + requisição + goods issue + branch receipt built (Phase 5 complete)** — `Branch`/`BranchMembership` (`operator`/`manager`/`admin`), `ActiveBranchMiddleware`, `/branch/select/` picker, `/branch/catalog/` (D37 unpriced default; cost always hidden, stock hint), `/branch/requests/` (requisição through `approved`; EUR caps only in priced mode), `/manage/internal-requests/` (goods issue, short-close), `/branch/receipts/` (branch receipt + branch stock); warehouse groups never imply branch access and vice versa
...
- Tests: `.venv/bin/python manage.py test products accounts procurement inventory branches orders threads company_voice inbox --noinput`
