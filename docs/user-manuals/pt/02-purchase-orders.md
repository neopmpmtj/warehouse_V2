...
2. Escolha o **Fornecedor** (A–Z pelo nome). A lista começa em `-----` — escolha um fornecedor; nenhum vem pré-selecionado.
...
3. Escolha o **Artigo** (A–Z pelo código — descrição visível; começa em `-----`, não num artigo do catálogo) e introduza uma **Quantidade** (número inteiro da unidade do artigo, maior que zero).
...
Marca o fornecedor como o fornecedor *preferido* para esse artigo. Defina-o em **Fornecedores → Preços de fornecedor** na gestão de artigos (marque **Principal**). Se mais tarde marcar outro preço de fornecedor como principal, o anterior é automaticamente desmarcado (só um principal por artigo). Na encomenda de compra, o custo vem do preço do fornecedor **desta encomenda** — nunca de outro fornecedor.
