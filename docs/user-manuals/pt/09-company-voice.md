...
Abra a engrenagem **Definições** (canto superior direito) para **Sessão iniciada como** e uma ligação pequena **Terminar sessão** na linha do título. **Ajuda** é o ícone azul **?** junto à engrenagem (placeholder). O **Idioma** é **Português** por defeito (o inglês é opcional); define-se no painel do pessoal (`/`) ou no painel da filial (`/branch/`) e memoriza-se neste browser.
