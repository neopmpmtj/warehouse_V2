...
2. Pick the **Supplier** (A–Z by name). The list starts on `-----` — choose a supplier; none is pre-selected.
...
3. Pick the **Item** (A–Z by the code — description shown; starts on `-----`, not a catalogue row) and enter a **Quantity** (whole number of the item’s unit, greater than zero).
...
It marks the supplier as the *preferred* supplier for that item. Set it from **Suppliers → Supplier prices** in the item console (tick **Primary**). If you later mark another supplier price as primary, the previous one is automatically unchecked (one primary per item). On a purchase order, the cost comes from **this order's supplier's** price — never another supplier's.
