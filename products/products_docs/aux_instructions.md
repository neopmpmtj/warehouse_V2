...
All server-side mutations go through `products/services.py` and are recorded in `ItemChangeLog`, `FamilyChangeLog`, or `SupplierChangeLog`.
...
reorder_level
vat_rate          (required FK to VatRate)
is_active         (soft delete; new items default inactive)
created_at
updated_at
```

Related tables: `FamilyProduct`, `Supplier`. There is no item–supplier link yet. Family and supplier names are case-insensitive unique.
...
- No stock or price on the item.

Do not bilingualise item data. Do not add images or extra columns unless requested.
...
```python
create_item(user, family, description, unit_of_measure, vat_rate, ...)
update_item(user, item, **fields)
deactivate_item(user, item, reason)   # reason required
reactivate_item(user, item, reason)   # reason required
get_items(active_only=True)
get_item_history(item)
create_family / update_family / get_family_history / get_families
create_supplier / update_supplier / get_supplier_history / get_suppliers
```

`create_item` always starts `is_active=False`. Activating is a separate `reactivate_item` (console Genesis).
...
It returns a simple HTML table (scaffold — phone UX is a later dedicated session). Staff use `/manage/items/` instead.
