...
(function () {
    ...
    function t(key, vars) {
        var dict = BRANCH_CONSUMPTION_I18N[currentLang()] || BRANCH_CONSUMPTION_I18N.en;
        var text = dict[key] !== undefined ? dict[key] : (BRANCH_CONSUMPTION_I18N.en[key] || key);
        if (vars) {
    ...
    function apiErrorMessage(payload) {
        var known = {
            branch_consume_reason_required: t("reasonRequired"),
            no_branch_lines_to_consume: t("needLine"),
            invalid_branch_consume_quantity: t("qtyRequired"),
        };
        if (payload && payload.code && known[payload.code]) {
            return known[payload.code];
        }
    ...
    function stockedItems() {
        return state.items.filter(function (item) {
            return parseInt(item.on_hand, 10) > 0;
        });
    }
    ...
    function resetForm() {
        ...
        var reason = document.getElementById("consume-reason");
        if (reason) {
            reason.value = "";
        }
        addLine();
    ...
    function renderHistory() {
        ...
        state.tickets.forEach(function (ticket) {
            ...
            tr.appendChild(el("td", formatDateTime(ticket.consumed_at)));
            tr.appendChild(el("td", ticket.consumed_by || t("dash")));
            tr.appendChild(el("td", ticket.reason || t("dash")));
            tr.appendChild(el("td", ticket.line_count));
            tr.appendChild(el("td", ticket.total_quantity));
    ...
    function selectTicket(id) {
        state.selectedId = id;
        renderHistory();
        api("/api/branch/consumption/" + id + "/").then(function (payload) {
            state.detail = payload.consumption;
            renderDetail();
        }).catch(function (err) {
    ...
    function loadStock() {
        return api("/api/branch/stock/").then(function (payload) {
            ...
            if (hint) {
                hint.hidden = stockedItems().length > 0;
    ...
    function loadHistory() {
        var path = "/api/branch/consumption/?page=" + state.page + "&page_size=" + state.pageSize;
        return api(path).then(function (payload) {
            state.tickets = payload.consumptions || [];
            if (payload.page) {
    ...
    function record() {
        ...
        api("/api/branch/consumption/", "POST", {
            reason: reason.value.trim(),
            lines: parsed,
        }).then(function (ticket) {
            showBanner(t("recorded"), true);
    ...
    function load() {
        ...
        Promise.all([loadStock(), loadHistory()]).then(function () {
            if (!document.querySelector("#line-body tr")) {
                addLine();
            }
            renderDetail();
        }).catch(function () {
